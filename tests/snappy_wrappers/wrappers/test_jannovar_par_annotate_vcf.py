...
from snappy_wrappers.wrappers.jannovar_par.annotate_vcf.parallel_annotate_vcf import (
    ParallelJannovarAnnotateVcfWrapper,
)
...
def snakemake_obj(minimal_config):
    ...
    rule_name = "variant_annotation_jannovar_annotate_vcf"
    threads = 2
    bench_iteration = 2
    script_dir = "/work"
    input_dict = {
...
def test_jannovar_wrapper_annotate_vcf_construct_merge_rule(
    snakemake_obj, variant_caller_fake_fs, mocker
):
    """Tests ParallelJannovarAnnotateVcfWrapper.construct_merge_rule()"""
    # Patch out file-system
    patch_module_fs("snappy_wrappers.wrapper_parallel", variant_caller_fake_fs, mocker)
    wrapper_par = ParallelJannovarAnnotateVcfWrapper(snakemake=snakemake_obj)
    # Define expected
    data_path = (Path(__file__).parent / "data/jannovar_par_annotate_vcf.snakemake").resolve()
