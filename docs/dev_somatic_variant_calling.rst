...
.. note::
    ...
    patterns) still apply, but the exact code references below are outdated.
    See the ``variant_calling`` step source code for the current implementation.
...
CUBI pipeline runs are invoked with the ``snappy run`` command that internally calls Snakemake with sensible defaults for either local execution or execution via SLURM on an HPC cluster.
...
.. COMMENTED-OUT literalinclude (module renamed): ../snappy_pipeline/workflows/somatic_variant_calling/Snakefile
    :language: python
    :lines: 1-10
...
This module-level documentation is also included into the user documentation, e.g., the one for the variant calling module is included at :ref:`step_variant_calling`.

.. COMMENTED-OUT literalinclude (module renamed): ../snappy_pipeline/workflows/somatic_variant_calling/__init__.py
    :language: python
    :lines: 1-4
