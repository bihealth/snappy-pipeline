...
# IMPORTANT NOTE-
# The GTF annotation file (snakemake.config[step_config][gene_expression_quantification][qc][rnaseqc_path_annotation_gtf])
# assumes that:
...
# Patch to fix errors (jar incompatible with java8): that must point to JAVA_HOME for a java1.7 installation
...
${{JAVA_HOME}}/bin/java -jar ${{jar_path}} \
    ...
    -t {snakemake.config[step_config][gene_expression_quantification][qc][rnaseqc_path_annotation_gtf]} \
    -s "Sample,{snakemake.input.bam}, " \
    ${{paired_cmd}} \
    -o ${{TMPDIR}}/rnaseqc \
    1> $(dirname {snakemake.log})/RNA-SeQC.out \
    2> $(dirname {snakemake.log})/RNA-SeQC.err
