# -*- coding: utf-8 -*-
"""Wrapper for running NGMLR (PacBio/Nanopore aligner)
"""
...
BAM_CHAIN=/fast/users/mholtgr/scratch/build/bamChain/bamChain

export TMPDIR=$(mktemp -d)
trap "rm -rf $TMPDIR" EXIT

mkdir -p $TMPDIR/out $TMPDIR/chained $TMPDIR/sorted $TMPDIR/sort.tmp
...
for fname in $(find $(dirname {snakemake.input}) -name '*.bam' -or -name '*.fast?.gz'); do
    ...
    | ngmlr \
        -t 16 \
        -x $preset \
        -r {snakemake.config[step_config][ngs_mapping][ngmlr][path_index]} \
        -q /dev/stdin \
        -o /dev/stdout \
    ...
    if [[ "{snakemake.params[chained]}" == "True" ]]; then
        python3 $BAM_CHAIN --minmapq 10 $TMPDIR/out/$i.bam $TMPDIR/chained/$i.bam
    else
        mv $TMPDIR/out/$i.bam $TMPDIR/chained/$i.bam
    fi

    samtools sort -m 4G -@ 3 \
        -o $TMPDIR/sorted/$i.bam \
        -T $TMPDIR/sort.tmp/ \
        $TMPDIR/chained/$i.bam
