...
from snappy_wrappers.wrapper_parallel import ParallelVariantAnnotationBaseWrapper  # noqa: E402
...
class ParallelGaktReadBackedPhasingWrapper(ParallelVariantAnnotationBaseWrapper):
    ...
    def __init__(self, snakemake):
        ...
        self.job_resources = ResourceUsage(
            threads=2,
            mem=f"{int(14.0 * self.get_job_mult_memory())}GB",
            runtime=f"{int(4 * self.get_job_mult_time())}h",
            partition=os.getenv("SNAPPY_PIPELINE_PARTITION"),
        )
        self.merge_resources = ResourceUsage(
            threads=2,
            mem=f"{int(2.0 * self.get_merge_mult_memory())}GB",
            runtime=f"{int(4 * self.get_merge_mult_time())}h",
            partition=os.getenv("SNAPPY_PIPELINE_PARTITION"),
        )
