...
compute_z_scores <- function(
    ...
    if (missing(df) || is.null(df) || !is.data.frame(df) || nrow(df)<1 ||
    ...
    if (is.null(gene_call_input_format) || !is.character(gene_call_input_format) || length(gene_call_input_format)!=1 || is.na(gene_call_input_format))
    ...
    if (any(has_both)) {
        ...
        gene_names <- gene_names[!is.na(gene_names$SYMBOL) & !is.na(gene_names$ENTREZID),]
...
read_featurecounts_files <- function(fns) {
    if (missing(fns) || !is.character(fns) || length(fns)==0 || sum(!is.na(fns) & fns!="")==0)
    ...
    tmp <- lapply(fns, function(x) {
        ...
        read.table(x, sep="\t", header=1, stringsAsFactors=FALSE)
    })
    ...
    counts <- matrix(as.numeric(NA), nrow=length(ids), ncol=length(tmp), dimnames=list(ids, fns))
...
#'
#' @examples
pipeline_to_entrez <- function(pip_counts, org_obj=org.Hs.eg.db::org.Hs.eg.db, pipeline_id="ENSEMBL") {
    if (missing(pip_counts) || is.null(pip_counts) || !is.matrix(pip_counts) || nrow(pip_counts)<1 || ncol(pip_counts)<1 ||
        !is.numeric(pip_counts) || is.null(rownames(pip_counts)) || any(is.na(rownames(pip_counts))) || any(rownames(pip_counts)==""))
        stop("Missing/illegal counts matrix")
    if (is.null(pipeline_id) || !is.character(pipeline_id) || length(pipeline_id)!=1 || is.na(pipeline_id) || pipeline_id=="")
        stop("Missing, illegal or unknown pipeline id type")
    if (pipeline_id == "ENTREZID") return(pip_counts)
    if (is.null(org_obj) || !is(org_obj, "OrgDb") || !all(c(pipeline_id, "ENTREZID") %in% AnnotationDbi::columns(org_obj)))
        stop("Missing/illegal organism database (OrgDb object)")
    ...
    if (length(i1)>0) {
        i1 <- unlist(i1)
        tmp <- pip_counts[i1,,drop=FALSE]
        rownames(tmp) <- names(i1)
        ent_counts <- rbind(ent_counts, tmp)
    }
    if (length(i2)>0) {
		tmp <- sapply(i2, function(j) apply(pip_counts[j,,drop=FALSE], 2, median))
		tmp <- t(matrix(as.vector(tmp), nrow=ncol(pip_counts), ncol=length(i2), dimnames=list(colnames(pip_counts), names(i2))))
        ent_counts <- rbind(ent_counts, tmp)
    }
...
normalise_counts <- function(counts, verbose=FALSE, fast=FALSE) {
    if (missing(counts) || is.null(counts) || !is.matrix(counts) || nrow(counts)<1 || ncol(counts)<1 ||
...
process_cnv_genes <- function(df) {
    ...
    if (any(duplicated(rslt$Entrez_Gene_Id))) {
        warning("Duplicated ENTREZ genes ", paste(unique(rslt$Entrez_Gene_Id[duplicated(rslt$Entrez_Gene_Id)]), collapse=", "))
...
z_scores <- function(norm_expr, cnv_call=NULL, min_nb_diploid=10) {
    if (missing(norm_expr) || is.null(norm_expr) || !is.matrix(norm_expr) || nrow(norm_expr)<1 || ncol(norm_expr)<1 ||
    ...
    if (is.null(min_nb_diploid) || !is.numeric(min_nb_diploid) || length(min_nb_diploid)!=1 || is.na(min_nb_diploid) || min_nb_diploid<0)
