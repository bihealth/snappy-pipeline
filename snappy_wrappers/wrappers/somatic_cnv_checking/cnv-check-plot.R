vcf_to_table <- function(vcf, sample, amplification=7) {
     ...
     x <- x |>
         ...
         dplyr::mutate(BAF=pmin(t_ref, t_alt)/(t_ref+t_alt)) |>
         dplyr::mutate(Call=factor("Neutral", levels=c("Deletion", "Loss", "Neutral", "Gain", "Amplification"))) |>
         dplyr::mutate(Call=replace(Call, CN == 0, "Deletion")) |>
         dplyr::mutate(Call=replace(Call, CN == 1, "Loss")) |>
         dplyr::mutate(Call=replace(Call, 2 < CN & CN < amplification, "Gain")) |>
         dplyr::mutate(Call=replace(Call, amplification <= CN, "Amplification"))
    x
}
...
plot_cnv <- function(x, scale=c("log2", "sqrt")) {
...
plot_locus <- function(x, genome_lengths) {
    p <- genome_lengths |>
