...
for line in sys.stdin:
    ...
    if line.startswith("#"):
        ...
        if (
            len(ref) != len(alts[0])
            or len(ref) < 2
            or (len(alts) > 1 and len(alts[1]) != len(ref))
            or (len(alts) > 2 and len(alts[2]) != len(ref))
            or len(alts) > 3
        ):
            print(line.strip())
        # Special treatment for multi-allelic sites
        elif len(alts) > 1:
            for newLine in splitMAVariant(
                chrom, pos, theId, ref, alts, qual, filters, info, theRest
            ):
                print(newLine)
        # Otherwise, split the variant into SNPs
        else:
            for newLine in splitVariant(
                chrom, pos, theId, ref, alts[0], qual, filters, info, theRest
            ):
                print(newLine)
