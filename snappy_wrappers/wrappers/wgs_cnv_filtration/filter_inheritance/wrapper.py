...
rec_hc_two_parents() {{
    ...
    tads_bed={snakemake.config[step_config][wgs_cnv_filtration][region_beds][all_tads]}

    # Create mother SV file
    exp_mother="(GT[$index_no] == \"alt\")"
...
# One key assumption here is that the variant should be exclusive to the family.

case "{snakemake.wildcards.inheritance}" in
    dominant)
        # Perform filtration for variants dominantly transmitted by father or mother
        filter="(BACKGROUND_CARRIERS == 0) && (GT[$index_no] == \"het\")"
        if [[ -z "$father_no" ]] && [[ ! -z "$mother_no" ]]; then
            filter+=" && (GT[$mother_no] == \"alt\")"
        elif [[ ! -z "$father_no" ]] && [[ -z "$mother_no" ]]; then
            filter+=" && (GT[$father_no] == \"alt\")"
        elif [[ ! -z "$father_no" ]] && [[ ! -z "$mother_no" ]]; then
            filter+=" && (((GT[$father_no] == \"alt\") && (GT[$mother_no] != \"alt\"))"
            filter+=" || ((GT[$father_no] != \"alt\") && (GT[$mother_no] == \"alt\")))"
        fi
    ...
    de_novo)
        ...
        simple_filter "(GT[$index_no] == \"alt\") && (UNAFFECTED_CARRIERS == 0) && (BACKGROUND_CARRIERS == 0)"
        ;;
    recessive_hom)
        # Perform filtration for homozygous variant present in both parents
        filter="(BACKGROUND_CARRIERS == 0) && (GT[$index_no] == \"alt\")"
        [[ ! -z "$father_no" ]] && filter+=" && (GT[$father_no] == \"alt\")"
        [[ ! -z "$mother_no" ]] && filter+=" && (GT[$mother_no] == \"alt\")"
        simple_filter "$filter"
        ;;
    recessive_hc)
        if [[ -z "$father_no" ]] || [[ -z "$mother_no" ]]; then
            rec_hc_one_parent "$index_no" "$father_no" "$mother_no"
        else
            rec_hc_two_parents "$index_no" "$father_no" "$mother_no"
        fi
    ;;
    *)  # else, "all"
