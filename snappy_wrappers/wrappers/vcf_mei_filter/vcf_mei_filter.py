#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Apply soft filters to MEI VCF file.
...
class GenotypeQualityAnnoFilterStep(FilterStep):
    """Annotate records with genotype qualities."""
    ...
    def _apply(self, record):
        ...
        for call in record.calls:
            call.data["LR"] = self._compute_lr(call.data.get("GL"))
        return record

    def _compute_lr(self, gl):
        if not gl or len(gl) < 3:
            return None
        else:
            r0 = gl[0] - gl[1]  # 0/1
            r1 = gl[0] - gl[2]  # 1/1
            return int(round(-1.0 * min(r0, r1)))
...
FILTERS = (
    # Add genotype quality column.
    GenotypeQualityAnnoFilterStep,
        ...
        header.add_format_line(
            vcfpy.OrderedDict(
                [
                    ...
                    (
                        "Description",
                        "Negative, log-10-scaled variant likelihood ratio (hom. over any var.)",
                    ),
                ]
            )
        )
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="(MELT) MEI VCF soft-filter application tool")
