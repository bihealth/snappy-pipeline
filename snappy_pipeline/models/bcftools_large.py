...
class BcfToolsVariantTypeAnnotate(enum.StrEnum):
    SNPS = "snps"
    INDELS = "indels"
    BOTH = "both"
    ALL = "all"
...
class BcfToolsVariantTypeCall(enum.StrEnum):
    SNPS = "snps"
    INDELS = "indels"


class BcfToolsVariantTypeConcat(enum.StrEnum):
    SNPS = "snps"
    INDELS = "indels"
    BOTH = "both"
    ALL = "all"
...
class BcfToolsVariantTypeIsec(enum.StrEnum):
    SNPS = "snps"
    INDELS = "indels"
    BOTH = "both"
    ALL = "all"
...
class BcfToolsVariantTypeMerge(enum.StrEnum):
    SNPS = "snps"
    INDELS = "indels"
    BOTH = "both"
...
class BcfToolsVariantTypeMergeUnobserved(enum.StrEnum):
    APPEND = "*"
    REMOVE = "**"


class BcfToolsVariantTypeNormDuplicates(enum.StrEnum):
    SNPS = "snps"
    INDELS = "indels"
    BOTH = "both"
    ALL = "all"
...
class BcfToolsVariantTypeNormMultiAllelic(enum.StrEnum):
    SNPS = "snps"
    INDELS = "indels"
    BOTH = "both"
    ANY = "any"
...
class BcfToolsVariantTypeView(enum.StrEnum):
    SNPS = "snps"
    INDELS = "indels"
    MNPS = "mnps"
    REF = "ref"
...
class BcfToolsVariantTypeFilter(enum.StrEnum):
    INDELS = "indels"
    MNPS = "mnps"
    BND = "bnd"
    OTHER = "other"
...
class BcfToolsVariantTypeStats(enum.StrEnum):
    SNPS = "snps"
    INDELS = "indels"
    BOTH = "both"
    ALL = "all"
...
class BcfToolsAnnotate(
    ...
    pair_logic: BcfToolsVariantTypeAnnotate = BcfToolsVariantTypeAnnotate.SOME
...
class BcfToolsConcat(SnappyModel, BcfToolsMixinOutput, BcfToolsMixinLocus):
    ...
    rm_dups: BcfToolsVariantTypeConcat | None = None
...
class BcfToolsIsec(SnappyModel, BcfToolsMixinOutput, BcfToolsMixinFilter, BcfToolsMixinLocus):
    collapse: list[BcfToolsVariantTypeIsec] = [BcfToolsVariantTypeIsec.NONE]
...
class BcfToolsMerge(SnappyModel, BcfToolsMixinOutput, BcfToolsMixinLocus):
    ...
    def ensure_valid_merge(self):
        MERGE_PATTERN = re.compile(
            r"^({})(,(\*{{1,2}}))?$".format("|".join(BcfToolsVariantTypeMerge))
        )
        if self.merge is not None:
            if not MERGE_PATTERN.match(self.merge):
                raise ValueError(f"Illegal merge option '{self.merge}'")
        return self
...
class BcfToolsNorm(SnappyModel, BcfToolsMixinOutput, BcfToolsMixinFilter, BcfToolsMixinLocus):
    ...
    rm_dup: BcfToolsVariantTypeNormDuplicates | None = None
...
class BcfToolsView(
    ...
    types: list[BcfToolsVariantTypeView] | None = None
    """Select comma-separated list of variant types: snps,indels,mnps,ref,bnd,other [null]"""
    exclude_types: list[BcfToolsVariantTypeView] | None = None
...
class BcfToolsCall(SnappyModel, BcfToolsMixinOutput, BcfToolsMixinLocus, BcfToolsMixinSample):
    ...
    skip_variants: BcfToolsVariantTypeCall | None = None
    """Skip indels/snps"""
...
class BcfToolsStats(SnappyModel, BcfToolsMixinFilter, BcfToolsMixinLocus, BcfToolsMixinSample):
    ...
    collapse: list[BcfToolsVariantTypeStats] = [BcfToolsVariantTypeStats.NONE]
    """Treat as identical records with <snps|indels|both|all|some|none>, see man page for details [none]"""
