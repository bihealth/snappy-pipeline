# -*- coding: utf-8 -*-
"""Implementation of the ``wgs_cnv_filtration`` step

This step takes annotated CNVs as the input from ``wgs_cnv_annotation`` and performs
post-filtration operations.

1. apply quality filter sets,
2. apply filters on the mode of inheritance, and
3. filter down the variants to specific regions.

Note that in contrast to the ``wgs_cnv_annotation`` step, variants are hard-filtered in this step.
...
.. include:: DEFAULT_CONFIG_wgs_cnv_filtration.rst
...
from ..wgs_cnv_annotation import WgsCnvAnnotationWorkflow
from ..wgs_cnv_calling import WgsCnvCallingWorkflow
...
DEFAULT_CONFIG = r"""
# Default configuration for wgs_cnv_filtration
step_config:
  wgs_cnv_filtration:
    ...
    tools_wgs_cnv_calling: False     # defaults to those in WGS CNV calling
    ...
    thresholds:                     # quality filter set, 'no_filter' is implicit
      conservative:
        index_pass: false
        index_pass_de_novo: false
        parent_pass: true
      conservative_de_novo:
        index_pass: true
        index_pass_de_novo: true
        parent_pass: true
    ...
    def get_input_files(self, action):
        ...
        def input_function(wildcards):
            ...
            for key, ext in zip(EXT_NAMES, EXT_VALUES):
                ...
                yield key, wgs_cnv_annotation(output_path) + ext
...
class WgsCnvFiltrationWorkflow(BaseStep):
    ...
    def __init__(
        ...
        if not self.config["tools_wgs_cnv_calling"]:
            self.config["tools_wgs_cnv_calling"] = self.w_config["step_config"]["wgs_cnv_calling"][
                "tools"
            ]
    ...
    def set_default_config(self):
        ...
        self.w_config["step_config"]["wgs_cnv_filtration"]["thresholds"]["no_filter"] = {
            "index_pass": False,
            "index_pass_de_novo": False,
            "parent_pass": False,
        }
