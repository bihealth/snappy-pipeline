...
step_config:
  varfish_export:
    ...
    path_sv_calling_targeted: null  # REQUIRED
    # Used output of WGS SV calling is variant calls
    ##NOT YET##path_sv_calling_wgs: null       # REQUIRED
...
class VarfishAnnotatorAnnotateStepPart(VariantCallingGetLogFileMixin, BaseStepPart):
    ...
    def _get_output_files_annotate(self):
        ...
        paths_work = {  # annotate will write out PED file
            "ped": f"{prefix}.ped",
        ...
        yield "output_links", [
            ...
            for work_path in chain(paths_work.values(), self.get_log_file("annotate").values())
        ]
    ...
    def _get_output_files_annotate_svs(self):
        ...
        for infix in infixes:
            yield "gts", f"{prefix}.gts.tsv.gz"
            yield "gts_md5", f"{prefix}.gts.tsv.gz.md5"
            yield "feature_effects", f"{prefix}.feature-effects.tsv.gz"
            yield "feature_effects_md5", f"{prefix}.feature-effects.tsv.gz.md5"
            yield "db_infos", f"{prefix}.db-infos.tsv.gz"
            yield "db_infos_md5", f"{prefix}.db-infos.tsv.gz.md5"
    ...
    def _get_output_files_bam_qc(self) -> SnakemakeDictItemsGenerator:
        prefix = (
            "work/{mapper}.varfish_export.{index_ngs_library}/out/"
            "{mapper}.bam_qc.{index_ngs_library}"
        )
        yield "bam_qc", f"{prefix}.bam-qc.tsv.gz"
        yield "bam_qc_md5", f"{prefix}.bam-qc.tsv.gz.md5"
...
class VarfishExportWorkflow(BaseStep):
    ...
    def check_config(self):
        self.ensure_w_config(
            ("step_config", "variant_export", "path_ngs_mapping"),
